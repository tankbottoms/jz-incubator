    ...
    ReentrancyGuard
{
    ...
    uint256 public initiative_goal;
    ...
    event Withdraw(address endowment, address[] _collaterals, uint256[] balances);
    ...
    event ClaimTokens(address indexed seller, uint256 amount);
     ...
     * @param _initiative_goal The amount sent to Endowment contract Endowment balance in ppm
    ...
    function initializeCurve(
        ...
        require(
            block.timestamp < _timeStart && _timeStart < _timeCooldown && _timeCooldown < _timeEnd,
            ERROR_BAD_TIMES
        );
        ...
        initiative_goal = _initiative_goal;
    ...
    function setTokenAndFundingGoal(address _token, uint256 _fundingGoal)
        external
        override
        onlyOwnerOrServiceProvider(owner(), _dao)
    {
        token = IERC20(_token);
    ...
    function withdraw() external timeClaimAndFundingGoal onlyOwnerOrServiceProvider(owner(), _dao) {
        ...
        bank.addToBalance(address(this), UNITS, bondingToken.totalSupply());
        ...
        for (uint256 i = 0; i < _collaterals.length; ) {
            ...
            if (tkn == address(0)) {
                ...
                balances[i] = balance;
                if (balance > 0) {
                    endowment.addERC20Balance{value: initiative_goal}(
                        movement,
                        address(0),
                        address(0),
                        initiative_goal,
                    ...
                    endowment.addERC20Balance{value: balance - initiative_goal}(
                        movement,
                        address(0),
                        address(0),
                        balance - initiative_goal,
                        IEndowment.BALANCE_TYPE.ENDOWMENT_BALANCE
                    );
                }
            } else {
...
*/
    ...
    function makeBuyOrder(
        ...
        uint256 collateralBalanceOfReserve = _balanceOf(address(reserve), _collateral).add(
    ...
    function evaluateBuyOrder(address _collateral, uint256 _depositAmount) public view returns (uint256 returnAmount) {
        ...
        uint32 reserveRatio = collaterals[_collateral].reserveRatio;
        returnAmount = formula.calculatePurchaseReturn(
            collateralSupply,
            collateralBalanceOfReserve,
            reserveRatio,
            depositAmountLessFee
    ...
    function evaluateSellOrder(address _collateral, uint256 _sellAmount) public view returns (uint256 returnAmount) {
        ...
        uint32 reserveRatio = collaterals[_collateral].reserveRatio;
        returnAmount = formula.calculateSaleReturn(
            collateralSupply,
            collateralBalanceOfReserve,
            reserveRatio,
            _sellAmount
    ...
    function _collateralValueIsValid(address _collateral, uint256 _value) internal view returns (bool) {
        if (_value == 0) {
            return false;
        }
